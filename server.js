...
app.post('/screenshot', async (req, res) => {
  ...
  try {
    ...
    // viewport ให้สูงขึ้นหน่อย กันรูปเตี้ยไป แต่ไม่ยาวเกิน
    ...
    // 5) แคปเฉพาะ viewport ตอนอยู่ล่างสุด (ไม่ใช้ fullPage แล้ว)
    const buffer = await page.screenshot({
      fullPage: fullPage === true ? true : false, // default: false (เพื่อ Telegram)
      type: 'jpeg',
      quality: 80,
    });
    ...
    return res
      .status(500)
      .json({ error: 'Screenshot failed', detail: err.message });
  } finally {
    if (browser) {
      await browser.close();
    }
